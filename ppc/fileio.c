...
Restart:
	...
	GetNewLine: // loop this far when comments consume entire line...
		...
		do
		{
			...
			{
				if( !current->format.position.offset.spaces && pft->line != current )
					current->format.position.offset.spaces = 1;
			}
...

		{
			...
			ContinueNoIncrement:
				...
				else if( pText[0] == '/'  )
				{
					...
					if( nStar ) // leading stars up to close...
					{
						...
						if( p->format.position.offset.spaces )
						{
							nStar = 0;
							continue;
						}
			...
			if( pft->bBlockComment ) // fell off end of line without a close on this.
			{
				...
				else
				{
					...
					if( g.flags.keep_comments )
					{
						PTEXT pOut = BuildLineEx( pNew, FALSE );
						if( pOut )
						{
							if( g.flags.bWriteLine )
							{
