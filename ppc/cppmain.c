...
void DumpSegs( PTEXT pOp )
{
	...
	while( tmp )
	{
		...
		{
			fprintf( stddbg, "[%d%s]", tmp->format.position.offset.spaces,GetText( tmp ) );
		}
		tmp = NEXTLINE( tmp );
	}
}
...
int ProcessSystemIncludeFile( char *name, int bAllowAbsolute, int bNext )
{
		...
		return TRUE;
	{
		LIST_FORALL( g.pUserIncludePath, idx, PTEXT, pPath )
		{
			...
			{
				if( idx )
					PPC_SetCurrentPath( GetText( pPath ) );
				return TRUE;
			}
		}
	...
	do
	{
		if( GetText( pWord )[0] == '\"' )
		{
			...
			if( !OpenNewInputFile( basename, Workname, GetCurrentFileName(), GetCurrentLine(), g.bAutoDepend  || g.flags.load_once, bNext ) )
			{
				...
				INDEX idx;
				int count;
				char *dir = (char*)pathrchr( g.pFileStack->longname );
				if( dir )
...
//==== PRAGMA =====================================================
		else if( TextLike( pDirective, "pragma" ) )
		{
			...
			{
				PTEXT pOut;
				pOut = BuildLineEx( NEXTLINE( pOp ), FALSE );
		...
		else if( TextLike( pDirective, "warning" ) )
		{
			...
			PTEXT pOut;
			pOut = BuildLineEx( GetCurrentWord(), FALSE );
		...
		{
			PTEXT pOut;
			pOut = BuildLineEx( pDirective, FALSE );
...
void ProcessFile( char *file, PVARTEXT pvt, uintptr_t shaderInfo )
{
	...
	g.current_shader_info   = shaderInfo;
	g.current_file_name     = file;
	g.first_binding_in_file = -1;
	g.first_in_location_in_file = -1;
	g.first_out_location_in_file = -1;
	g.pvtOut                    = pvt;
	{
		INDEX idx;
		PLOCATION_BINDING plb;
		LIST_FORALL( g.pLocations, idx, PLOCATION_BINDING, plb ) {
			Release( plb->varType );
			Release( plb );
			SetLink( &g.pLocations, idx, NULL );
		}
		LIST_FORALL( g.pBindings, idx, PLOCATION_BINDING, plb ) {
			Release( plb->varType );
			Release( plb );
			SetLink( &g.pBindings, idx, NULL );
		}
	}
	if( !pvt ) {
...
void ReleaseIncludePaths( void )
{
	INDEX idx;
	PTEXT path;
	LIST_FORALL( g.pSysIncludePath, idx, PTEXT, path )
		Release( path );
	DeleteList( &g.pSysIncludePath );

	LIST_FORALL( g.pUserIncludePath, idx, PTEXT, path )
	{
...
#ifdef PPC_LIBRARY
int GetFirstBinding( void ) {
	return g.first_binding_in_file;
}

int GetFirstLayout( void ) {
	return g.first_binding_in_file;
}
