...
// what it ended up using as a first-binding per-file...
	...
	, DefineBindingDeclaration 
// __LAYOUT__ is an auto-incrementing counter that starts at 0
// it should also take some sort of external structure to report
// what it ended up using as a first-binding per-file...
	, DefineLocationIn // static symbol..
	, DefineLocationOut // static symbol..
     , DefineLocationDeclaration // builds layout(location=0) (dir) (type) (name); and
                             // keeps names of type and name in params
...
void FixQuoting( PTEXT test )
{
	while( test )
	{
		if( GetText( test )[0] == '\"' )
		{
			...
			insert->format.position.offset.spaces = test->format.position.offset.spaces;
			test->format.position.offset.spaces = 0;
...
void DeleteDefineContent( PDEF p )
{
		...
		LineRelease( p->pData );
	{
		INDEX idx;
		PTEXT param;
		LIST_FORALL( p->pParams, idx, PTEXT, param )
		{
			LineRelease( param );
		}
...
void InitDefines( void )
{
	...
	{
		DECLTEXT( constname, "__LOCATION_out__" );
		DefineLocationOut.pName = (PTEXT)&constname;
		DefineLocationOut.pParams = NULL;
		DefineLocationOut.bUsed = FALSE;
		DefineLocationOut.pLesser  = NULL;
		DefineLocationOut.pGreater = NULL;
	}
	{
		DECLTEXT( constname, "__LOCATION_in__" );
		DefineLocationIn.pName = (PTEXT)&constname;
		DefineLocationIn.pParams = NULL;
		DefineLocationIn.bUsed = FALSE;
		DefineLocationIn.pLesser  = NULL;
		DefineLocationIn.pGreater = NULL;
	}
	{
		DECLTEXT( constname, "Location" );
		DefineLocationDeclaration.pName   = (PTEXT)&constname;
		DefineLocationDeclaration.pParams = NULL;
		DefineLocationDeclaration.bUsed   = FALSE;
		AddLink( &DefineLocationDeclaration.pParams, SegCreateFromText( "dir" ) );
		AddLink( &DefineLocationDeclaration.pParams, SegCreateFromText( "type" ) );
		AddLink( &DefineLocationDeclaration.pParams, SegCreateFromText( "name" ) );
		PTEXT tmp1, tmp2;
		DefineLocationDeclaration.pData = SegAppend(
		     SegAppend( SegAppend( SegCreateFromText( "layout(location=" ),
		                           SegAppend( SegCreateFromText( "__LOCATION_##dir##__" ),
		                                      SegCreateFromText( ") " ) ) ),
		                SegAppend( SegCreateFromText("dir"), tmp1=SegCreateFromText( "type" ) ) ),
		     tmp2=SegCreateFromText( "name" ) );
		tmp1->format.position.offset.spaces = 1;
		tmp2->format.position.offset.spaces = 1;
		DefineLocationDeclaration.pLesser = NULL;
		DefineLocationDeclaration.pGreater = NULL;
	}
	{
	...
	{
		DECLTEXT( constname, "Binding" );
		DefineBindingDeclaration.pName = (PTEXT)&constname;
		DefineBindingDeclaration.pParams = NULL;
		DefineBindingDeclaration.bUsed   = FALSE;
		PTEXT tmp1, tmp2;
		AddLink( &DefineBindingDeclaration.pParams, SegCreateFromText( "dir" ) );
		AddLink( &DefineBindingDeclaration.pParams, SegCreateFromText( "type" ) );
		AddLink( &DefineBindingDeclaration.pParams, SegCreateFromText( "name" ) );
		DefineBindingDeclaration.pData = SegAppend(
		     SegAppend( SegAppend( SegCreateFromText( "layout(binding=" ),
		                           SegAppend( SegCreateFromText( "__BINDING__" ),
		                                      SegCreateFromText( ") " ) ) ),
		                SegAppend( SegCreateFromText("dir"), tmp1=SegCreateFromText( "type" ) ) ),
		     tmp2=SegCreateFromText( "name" ) );
		tmp1->format.position.offset.spaces = 1;
		tmp2->format.position.offset.spaces = 1;
		DefineBindingDeclaration.pLesser    = NULL;
		DefineBindingDeclaration.pGreater = NULL;
	}
	{
	...
	//if( TextIs( pName, "__TIME__ " ) ) // hh:mm:ss
	{
		...
		DefineTime.pData->format.position.offset.spaces = 1;
	...
	//if( TextIs( pName, "__DATE__ " ) ) // Mmm dd yyyy dd = ' x' if <10
	{
		...
		DefineDate.pData->format.position.offset.spaces = 1;
...
#ifdef DEFINE_STDC_VERSION
	{
		...
		DefineStdCVersion.pData->format.position.offset.spaces = 1;
...
PDEF FindDefineName( PTEXT pName, int params )
{
	...
	{
		if( g.first_binding_in_file < 0 ) g.first_binding_in_file = g.binding;
		if( DefineLine.pData )
			LineRelease( DefineLine.pData );
		DefineLine.pData = SegCreateFromInt( g.binding++ );
		DefineLine.bUsed = FALSE;
		
	}
	else if( TextIs( pName, "__LOCATION_out__" ) )
	{
		if( g.first_out_location_in_file < 0 )
			g.first_out_location_in_file = g.out_location;
		if( DefineLine.pData )
			LineRelease( DefineLine.pData );
		DefineLine.pData = SegCreateFromInt( g.out_location++ );
		DefineLine.bUsed = FALSE;
	} else if( TextIs( pName, "__LOCATION_in__" ) ) {
		if( g.first_in_location_in_file < 0 )
			g.first_in_location_in_file = g.in_location;
		if( DefineLine.pData )
			LineRelease( DefineLine.pData );
		DefineLine.pData = SegCreateFromInt( g.in_location++ );
		DefineLine.bUsed = FALSE;
...
int ProcessDefine( int type )
{
	...
	if( !pCurrentDefine )
	{
		...
		if( ( NEXTLINE( pWord )
		  && ( NEXTLINE( pWord )->format.position.offset.spaces == 0 ) 
		  && ( NEXTLINE( pWord )->format.position.offset.tabs == 0 ) 
		    ) )
		{
					...
					if( GetText( pWord )[0] != ',' )
					{
						...
						else
						{
							...
							if( !pCurrentDefine->bVarParams )
							{
								...
								LIST_FORALL( pCurrentDefine->pParams, idx, PTEXT, param )
								{
	...
	if( GetCurrentWord() )
	{
		...
		while( p )
		{
			...
			if( p == GetCurrentWord() )
				newseg->format.position.offset.spaces = 0;
			pCurrentDefine->pData = SegAppend( pCurrentDefine->pData
													  , newseg );
			p = NEXTLINE(p);
		}
		pCurrentDefine->pData->format.position.offset.spaces = 0;
	}
	if( g.bDebugLog & DEBUG_DEFINES )
	{
...
PTEXT BuildSizeofArgs( PTEXT args )
{
	...
	for( ; args; args = NEXTLINE( args ) )
	{
		...
		if( text[0] == ',' )
		{
			...
			{
				vtprintf( g.pvt , " ( sizeof( " );
				VarTextEnd( g.pvt );
				out = trinary_then;
				while( out )
				{
					vtprintf( g.pvt, "%s", GetText( out ) );
					tmp = VarTextEnd( g.pvt );
					tmp->flags |= out->flags & TF_NOEXPAND;
					tmp->format.position.offset.spaces = out->format.position.offset.spaces;
					out = NEXTLINE( out );
				}
				vtprintf( g.pvt, " ) > " );
				VarTextEnd( g.pvt );
			...
			vtprintf( g.pvt, " + 3 ) / 4 ) * 4 " );
			VarTextEnd( g.pvt );
			vtprintf( g.pvt, ")" );
			VarTextEnd( g.pvt );
			vtprintf( g.pvt, "+" );
			VarTextEnd( g.pvt );
			result = SegAppend( result, VarTextGet( g.pvt ) );
		}
		else
		{
			if( trinary_collect_else )
...
INDEX FindArg( PLIST pArgs, PTEXT pName )
{
	...
	LIST_FORALL( pArgs, i, PTEXT, pTest )
	{
...
void EmptyArgList( PLIST *pArgVals )
{
	INDEX idx;
	PTEXT pDelete;
	LIST_FORALL( *pArgVals, idx, PTEXT, pDelete )
	{
		LineRelease( pDelete );
	}
...
#define SegSubstRange( t, e, th ) SegSubstRangeEx( t, e, th DBG_SRC )
	PTEXT _this     = *pp_this;
	...
	if( !that ) {
		...
		PTEXT that_start = that, that_end = that;
	...
	LineReleaseEx( pp_this[0] DBG_RELAY );
	pp_this[ 0 ] = 0;
	if( that )
		*pp_this = that;
	else
		 ...
		 GetText( pWord )[0] == '\"' )
	{
				 ...
				 GetText( pWord )[0] == '(' )
			{
					 ...
					 GetText( pWord )[0] != ')' )
				{
					if( FindDefineName( pWord, IGNORE_PARAMS ) )
					{
						...
						pNewWord->format.position.offset.spaces = pStart->format.position.offset.spaces;
					}					else
					{
		...
		if( pDefine && !pDefine->bUsed ) // if we found the name as a symbol...
		{
			...
			if( pDefine->pParams )
			{
				...
				while(1)
				{
					...
					if( !bVarArg )
					{
						if( pVal )
						{
							pVal->format.position.offset.spaces = 0;
							if( g.bDebugLog & DEBUG_SUBST )
							{
					...
					for( p = pDefine->pData; p; p = NEXTLINE( p ) )
					{
						...
						if( idx == INVALID_INDEX )
						{
							...
							if( MakeString == 2 )
							{
								if( seg )
								{
									...
									vtprintf( g.pvt, "%s%s", GetText( valnow ), GetText( seg ) );
									newseg = VarTextGet( g.pvt );
									newseg->format.position.offset.spaces = valnow->format.position.offset.spaces;
									SegGrab( valnow );
									if( valnow == pSubst )
										pSubst = NULL;
									LineRelease( valnow );
							...
							else if( MakeString == 1 )
							{
								...
								{
									PTEXT tmp;
									VarTextAddCharacter( g.pvt, '\"' );
									tmp = VarTextEnd( g.pvt );
									if( !(tmp->format.position.offset.spaces = p->format.position.offset.spaces ) )
										tmp->format.position.offset.spaces++;

									vtprintf( g.pvt, "%s", GetText( seg ) );
									VarTextEnd( g.pvt );
									VarTextAddCharacter( g.pvt, '\"' );
									LineRelease( seg );
						...
						else
						{
							...
							if( MakeString == 2 )
							{
								...
								if( seg )
								seg->format.position.offset.spaces = 0;
								*/
								MakeString = 0;
							}
							else if( MakeString == 1 )
			...
			else // if !define->params...
			{
				...
				if( pSubst = TextDuplicate( pDefine->pData, FALSE ) )
				{
					if( g.bDebugLog & DEBUG_SUBST )
						fprintf( stddbg, "First subst word: %s\n", GetText( pSubst ) );
					pSubst->format.position.offset.spaces = 1;
				}
				else
				{
					if( g.bDebugLog & DEBUG_SUBST )
			...
			if( pSubst )
			{
				...
				if( pSubst )
					pReset = pSubst;
				if( pDefine == &DefineLocationDeclaration ||
					pDefine == &DefineBindingDeclaration )
				{
					// leave a blank...
					// do something... 
			...
			if( pDefine == &DefineLocationDeclaration ) {
				PLOCATION_BINDING binding = New( struct location_binding_tag );
				PTEXT dir                 = GetLink( &pArgVals, 0 );
				char const *dir_text      = GetText( dir );
				if( dir_text[ 0 ] == 'i'
				    && dir_text[ 1 ] == 'n'
				    && dir_text[ 2 ] == 0 ) {
					binding->type = LOCATION_TYPE_IN;
				}
				else if( dir_text[ 0 ] == 'o'
						 && dir_text[ 1 ] == 'u'
						 && dir_text[ 2 ] == 't'
						 && dir_text[ 3 ] == 0 ) {
					binding->type = LOCATION_TYPE_OUT;
				} else {
					fprintf( stderr,
					         "%s(%d): Invalid direction specifier for layout "
					         "location: %s\n",
					         GetCurrentFileName(), GetCurrentLine(), dir_text );
					binding->type = LOCATION_TYPE_IN; // default to something
				}
				StrCpyEx( binding->name, GetText( GetLink( &pArgVals, 2 ) ), 64 );
				binding->varType    = StrDup( GetText( GetLink( &pArgVals, 1 ) ) );
				binding->file       = g.current_file_name;
				binding->shaderInfo = g.current_shader_info;
				AddLink( &g.pLocations, binding );
			} else if( pDefine == &DefineBindingDeclaration ) {
				PLOCATION_BINDING binding = New( struct location_binding_tag );
				PTEXT dir                 = GetLink( &pArgVals, 0 );
				char const *dir_text      = GetText( dir );
				binding->type             = LOCATION_TYPE_UNIFORM;
				StrCpyEx( binding->name, GetText( GetLink( &pArgVals, 2 ) ), 64 );
				binding->varType    = StrDup( GetText( GetLink( &pArgVals, 1 ) ) );
				binding->file       = g.current_file_name;
				binding->shaderInfo = g.current_shader_info;
				AddLink( &g.pBindings, binding );
			}
